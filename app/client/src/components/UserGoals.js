...
class UserGoals extends Component {
    ...
    onSave = (id, newVal, oldVal, cheked) => {
        const { onEdit } = this.props;

        if (onEdit) onEdit(id, newVal || oldVal, cheked);
    };
                        ...
                        {data.map((x, i) => {
                            return (
                                <div key={i} onDragOver={() => this.onDragOver(i)}>
                                    <div
                                        className='d-flex flex-row task-row'
                                        ...
                                        {account._id === x.userId ? (
                                            <React.Fragment>
                                                <span className='mt-1 mr-2'>
                                                    <a href='javascript:;' className='nounderline text-dark'>
                                                        <Tooltip title='Move Task'>
                                                            <i className='fa fa-grip-horizontal' aria-hidden='true' />
                                                        </Tooltip>
                                                    </a>
                                                </span>
                                                <span className='mt-1 mr-2'>
                                                    <a href='javascript:;' onClick={() => this.onDelete(x._id)} className='nounderline text-dark'>
                                                ...
                                                <div className='mt-1 mr-2'>
                                                    <DatePicker
                                                        ...
                                                        onChange={date =>
                                                            this.changeGoalData(x._id, {
                                                                checked: x.checked,
                                                                taskDate: moment(date).format('YYYYMMDD'),
                                                            })
                                                        }
                                                    />
                                                </div>
                                                <div className='mt-1 mr-2'>
                                                    <span>
                                                        <a
                                                            href='javascript:;'
                                                            onClick={() => this.onShowPriorityBox(i, true)}
                                                            className='nounderline text-dark'>
                                                            <Tooltip title='Choose Priority'>
                                                    ...
                                                    {showPriorityBox[i] && (
                                                        <React.Fragment>
                                                            <select
                                                                className='prioritybox-selector'
                                                                value={x.priority}
                                                                onChange={e =>
                                                                    this.changeGoalData(x._id, { checked: x.checked, priority: e.target.value })
                                                                }>
                                                            ...
                                                            <div className='prioritybox-wrapper' onClick={() => this.onShowPriorityBox(i, false)} />
                                                        </React.Fragment>
                                                    )}
                                                </div>
                                            </React.Fragment>
                                        ...
                                        <div className='newTask'>
                                            {x.priority && <span className={`priority ${x.priority}`}>{x.priority}</span>}
                                            ...
                                            {account._id === x.userId ? (
                                                <Tooltip title='Click me to modify'>
                                                    <div>
                                                        <EasyEdit
                                                            type='text'
                                                            value={x.task}
                                                            onSave={val => this.onSave(x._id, val, x.task, x.checked)}
                                                            onValidate={e => (e ? true : false)}
                                                        />
                                                    </div>
                                                </Tooltip>
                                            ) : (
